...
/**
 * @file
 * Import Moveable Type blog to Drupal 5.
 *
...
$_SERVER['SCRIPT_NAME'] = '/mt2drupal.php';
$_SERVER['SCRIPT_FILENAME'] = '/mt2drupal.php';
...
$xml = simplexml_load_file('/tmp/files/netadmin/content/export/cat-hank_hr_support-export.xml');
...
foreach($xml->item as $item) {
  ...
  $node->status = 1;
  $node->promote = 0; // in_array('promote', $node_type_default);
  $node->sticky = 0;
  $node->parent = 212;
  ...
  // $node->uid = $user->uid;
  // $node->name = $user->name;
  // This works if users have been created in Drupal. Otherwise sets author uid=0.
  ...
  $node->body = check_markup(str_replace($oldurl, '/', $item->description));
  ...
  echo "Import: ". str_replace($oldurl, '', $item->link) ." \"$item->title\" by $node->name (uid=$node->uid)\n";
    ...
    if ($filename = $image->getAttribute('src')) {
      if (file_exists($filedir . $filename)) {
        ...
        if ($file_obj = mt2file($filedir . $filename, $node->uid)) {
          ...
          $image->setAttribute('src', "/hank/". $file_obj->filepath);
        }
        else {
          echo " -- IMPORT FAILED\n";
        }
    ...
    if ($filename = $link->getAttribute('href')) {
      if (file_exists($filedir . $filename)) {
        ...
        if ($file_obj = mt2file($filedir . $filename, $node->uid)) {
          $node->files[$file_obj->fid] = $file_obj;
          echo " -- created ". $file_obj->fid ." at ". $file_obj->filepath ."\n";
          $link->setAttribute('href', "/hank/". $file_obj->filepath);
        }
        else {
          echo " -- IMPORT FAILED\n";
        }
  ...
  $newnode->uid = db_result(db_query("SELECT uid from {users} WHERE LOWER(name) = '%s' LIMIT 1",$item->creator));
  $newnode->name = $item->creator;
  node_save($newnode);
...
function mt2file($file, $uid) {
  /*
   * This portion of the code based on the example by Jamie McClelland found at
   * http://current.workingdirectory.net/posts/2009/attach-file-to-node-drupal-6/
   */
  ...
  if (file_copy($file, $dest, FILE_EXISTS_REPLACE)) {
    ...
    // Scale image uploads.
    // $file_obj = _upload_image($file_obj);
    ...
    // preview or attach. See notes in upload.module source.
